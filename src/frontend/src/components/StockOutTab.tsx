...
export default function StockOutTab() {
  ...
  const { data: stockOutTransactions } = useGetStockTransactionsByType('stockOut');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const transaction: StockTransaction = {
      ...
      transactionType: { stockOut: null } as any,
      reason,
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="bg-card p-6 rounded-lg border border-border space-y-4">
        <h2 className="text-xl font-semibold">Record Stock Out</h2>
        ...
        >
          {recordTransactionMutation.isPending ? 'Recording...' : 'Record Stock Out'}
        </Button>
      </form>

      <div>
        <h3 className="text-lg font-semibold mb-4">Stock Out History</h3>
        <StockTransactionTable transactions={stockOutTransactions || []} />
      </div>
    </div>
  );
}
